$(document).ready(function () {
    ...
    $(document).on("click", '.btnMensagem', function () {
        if (isMobile.any()) {
            ...
            alert("Impossivel compartilhar via whatsapp em um computador!");
        }
    });

    somaTotal = function () {
        var qtds = document.getElementsByClassName("qtdPedProd");
        var precos = document.getElementsByClassName("precoProd");

        var tam = qtds.length;
        var soma = 0;

        for (var i = 0; i < tam; i++) {
            var mult = qtds[i].getAttribute('value') * precos[i].getAttribute('value');
            soma += mult;
        }

        document.getElementById("txt_total").value = soma;
    };
    ...
    AddTableRow = function (id, nome, preco, qtd) {
        $('#table_itemPedido').append("<tr><td> <input class='idPedProd' type='text' value=" + id + " readonly/> </td><td>" + nome + "</td><td><input class='precoProd' type='number' value=" + preco + " readonly/></td><td><input class='qtdPedProd' type='number' id='txt_qtdProd' value=" + qtd + " /></td><td><a data-role='button' class='ui-btn ui-shadow ui-corner-all ui-icon-delete ui-btn-icon-notext' onclick='RemoveTableRow(this); return false;' id='btn_removerItem'>Remover</a></td></tr>");
    };

    $("#btn_addItem").click(function (e) {
        var id_produto = $('#txt_produtos option:selected').val();
        var nome_preco = $('#txt_produtos option:selected').text().split('-');

        e.preventDefault();

        AddTableRow(id_produto, nome_preco[0], nome_preco[1], 1);
    });

    $(document).on("click", '#btn_calcTotal', function () {
        somaTotal();
        return false;
    });
    ...
    removerDup = function() {
        var seen = {};
        $('#table_produtosDoDia tr').each(function() {
            var txt = $(this).text();
            if (seen[txt])
                $(this).remove();
            else
        ...
        console.log("executou o remover dup");
        console.log(seen);
    };
    ...
    salvarPDF = function() {
        ...
        document.getElementById("relHeader").innerHTML += "<h3>Fornecedor: " + $('#txt_fornecedores').children("option:selected").text() + "</h3>";

        doc.addHTML($('#relatorioCompras'), function() {
            doc.save("Rel. Produtos Vendidos - " + $('#txt_fornecedores').children("option:selected").text() + " - " + data.getDate() + "/" + (data.getMonth() + 1) + "/" + data.getFullYear() + ".pdf");
        });
    };

    converterData = function(date){
        var d = new Date(date);
...
function abrirFornecedores(id, nome, telefone) {
    document.getElementById("txt_idForn").value = id;
    document.getElementById("txt_nomeForn").value = nome;
    document.getElementById("txt_telForn").value = telefone;

    window.location = "index.html#cadFornecedor";
}

function abrirClientes(id, nome, telefone, dt, obs) {
    document.getElementById("txt_idCli").value = id;
    document.getElementById("txt_nomeCli").value = nome;
    document.getElementById("txt_telefoneCli").value = telefone;
    document.getElementById("txt_nascimentoCli").value = dt;
    document.getElementById("txt_obsCli").value = obs;

    window.location = "index.html#cadCliente";
}

function abrirProdutos(id, nome, preco, id_forn, durabilidade, qtd) {
    document.getElementById("txt_codProd").value = id;
    document.getElementById("txt_nomeProd").value = nome;
    document.getElementById("txt_precoProd").value = preco;
    document.getElementById("txt_durabilidadeProd").value = durabilidade;
    document.getElementById("txt_qtdProd").value = qtd;
    document.getElementById("txt_fornecedoresProd").value = id_forn;

    window.location = "index.html#cadProduto";
}

function abrirPedidos(id, total, prazo, pago, id_cliente) {
    document.getElementById("txt_idPed").value = id;
    document.getElementById("txt_total").value = total;
    document.getElementById("txt_prazo").value = prazo;
    
    if(pago == 1)
        document.getElementById("txt_pago").click();

    document.getElementById("txt_clientes").value = id_cliente;
    ...
    window.location = "index.html#cadPedido";
}